...
export class FornecedorUpdateComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog,private router: ActivatedRoute,private fornecedorService: FornecedorService, private route: Router, private dateAdapter: DateAdapter<Date>, private ufService: UfService, private cidadeService: CidadeService) {
  ...
  getByIdFornecedor(id:number){
    this.fornecedorService.getByIdFornecedor(id).subscribe((data) => {
      this.fornecedorForm = Object.values(data)[0];
    })
  }
  ...
  updatefornecedor(){
    this.isLoading = true;
    setTimeout(() => {
      this.fornecedorService.updateFornecedor(this.fornecedorForm).subscribe((data) => {
