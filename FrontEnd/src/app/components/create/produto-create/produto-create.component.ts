...
export class ProdutoCreateComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog,private produtoService: ProdutoService, private route: Router, private dateAdapter: DateAdapter<Date>) {
  ...
  onInputChange(newValue: number) {
    setTimeout(() => {
      if (newValue > this.valorFinal) {
        this.produtoForm.quantidade_minima = this.valorFinal - 1; // Define o valor limite no campo de entrada
      }
    }, 100);
  }
  ...
  onInputChanges(newValue: number) {
    setTimeout(() => {
      if (newValue > this.valorFinal) {
        this.produtoForm.entrega = this.valorFinal - 1; // Define o valor limite no campo de entrada
      }
    }, 100);
  }
  ...
  createProduto(){
    this.isLoading = true;
    setTimeout(() => {
      this.produtoService.createProduto(this.produtoForm).subscribe((data) => {
